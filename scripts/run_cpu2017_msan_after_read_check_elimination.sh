#!/usr/bin/env bash

set -euo pipefail

cd /home/dragon/DESAN

cp spec_configs/cpu2017-desan.cfg spec/cpu2017-v1.0.5/config/desan.cfg
set +u
pushd spec/cpu2017-v1.0.5 >/dev/null
source ./shrc
popd >/dev/null
set -u

export LLVM_CONFIG="${LLVM_CONFIG:-llvm-config-18}"
export OPT="${OPT:-opt-18}"
export CLANG="${CLANG:-clang-18}"
export CLANGXX="${CLANGXX:-clang++-18}"
export PLUGIN="${PLUGIN:-/home/dragon/DESAN/build/DESANPass.so}"
export DESAN_SPEC_FALLBACK="${DESAN_SPEC_FALLBACK:-1}"
export DESAN_SPEC_PASS_TIMEOUT="${DESAN_SPEC_PASS_TIMEOUT:-900}"
export DESAN_SPEC_QUIET="${DESAN_SPEC_QUIET:-1}"
export DESAN_PASS_ARGS="${DESAN_PASS_ARGS:--desan-core-top-n=0 -desan-core-min-ratio=0}"
# SPEC workloads can contain benign uninitialized-value flows. Keep MSan
# instrumentation overhead for base/DESAN comparison, but prevent reports from
# aborting the run before timings are emitted.
export MSAN_OPTIONS="${MSAN_OPTIONS:-halt_on_error=0:exit_code=0:report_umrs=0:print_stats=0}"

copy_latest_results() {
  local out_dir="$1"
  mkdir -p "${out_dir}/spec-results"

  local set latest
  for set in intrate intspeed fprate fpspeed; do
    latest="$(ls -t spec/cpu2017-v1.0.5/result/CPU2017.*.${set}.test.csv 2>/dev/null | head -n 1 || true)"
    if [[ -n "${latest}" ]]; then
      cp "${latest}" "${out_dir}/spec-results/"
    fi
  done
}

has_cpu2017_results() {
  local out_dir="$1"
  compgen -G "${out_dir}/spec-results/CPU2017.*.intrate.test.csv" >/dev/null &&
    compgen -G "${out_dir}/spec-results/CPU2017.*.intspeed.test.csv" >/dev/null &&
    compgen -G "${out_dir}/spec-results/CPU2017.*.fprate.test.csv" >/dev/null &&
    compgen -G "${out_dir}/spec-results/CPU2017.*.fpspeed.test.csv" >/dev/null
}

run_one() {
  local label="$1"
  echo "=== CPU2017 ${label} ==="
  set +e
  runcpu --config=desan --size=test --tune=base --iterations=1 --rebuild all
  local status=$?
  set -e
  echo "=== CPU2017 ${label} status=${status} ==="
  return 0
}

NATIVE_OUT=/home/dragon/DESAN/out/spec-cpu2017-native
BASE_OUT=/home/dragon/DESAN/out/spec-cpu2017-msan-base
DESAN_OUT=/home/dragon/DESAN/out/spec-cpu2017-msan-after-read-check-elimination
SUMMARY_MD=/home/dragon/DESAN/out/spec-cpu2017-msan-after-read-check-elimination-summary.md
SUMMARY_CSV=/home/dragon/DESAN/out/spec-cpu2017-msan-after-read-check-elimination-summary.csv

run_native="${RUN_NATIVE:-auto}"
run_base="${RUN_BASE:-auto}"
run_desan="${RUN_DESAN:-1}"

if [[ "${run_native}" == "1" ]] ||
   { [[ "${run_native}" == "auto" ]] && ! has_cpu2017_results "${NATIVE_OUT}"; }; then
  rm -rf "${NATIVE_OUT}"
  mkdir -p "${NATIVE_OUT}"
  export DESAN_OUT_DIR="${NATIVE_OUT}"
  export DESAN_SPEC_SANITIZER=none
  export DESAN_SPEC_DISABLE_PASS=1
  run_one native
  copy_latest_results "${NATIVE_OUT}"
fi

if [[ "${run_base}" == "1" ]] ||
   { [[ "${run_base}" == "auto" ]] && ! has_cpu2017_results "${BASE_OUT}"; }; then
  rm -rf "${BASE_OUT}"
  mkdir -p "${BASE_OUT}"
  export DESAN_OUT_DIR="${BASE_OUT}"
  export DESAN_SPEC_SANITIZER=msan
  export DESAN_SPEC_DISABLE_PASS=1
  run_one msan-base
  copy_latest_results "${BASE_OUT}"
fi

if [[ "${run_desan}" == "1" ]]; then
  rm -rf "${DESAN_OUT}"
  mkdir -p "${DESAN_OUT}"

  export DESAN_OUT_DIR="${DESAN_OUT}"
  export DESAN_SPEC_SANITIZER=msan
  export DESAN_SPEC_DISABLE_PASS=0
  run_one msan-after-read-check-elimination
  copy_latest_results "${DESAN_OUT}"
fi

python3 scripts/spec_asan_summary.py \
  --suite CPU2017 \
  --records "${DESAN_OUT}/spec-logs/compile_records.tsv" \
  --native-csv "${NATIVE_OUT}/spec-results/CPU2017.*.intrate.test.csv" \
  --native-csv "${NATIVE_OUT}/spec-results/CPU2017.*.intspeed.test.csv" \
  --native-csv "${NATIVE_OUT}/spec-results/CPU2017.*.fprate.test.csv" \
  --native-csv "${NATIVE_OUT}/spec-results/CPU2017.*.fpspeed.test.csv" \
  --before-csv "${BASE_OUT}/spec-results/CPU2017.*.intrate.test.csv" \
  --before-csv "${BASE_OUT}/spec-results/CPU2017.*.intspeed.test.csv" \
  --before-csv "${BASE_OUT}/spec-results/CPU2017.*.fprate.test.csv" \
  --before-csv "${BASE_OUT}/spec-results/CPU2017.*.fpspeed.test.csv" \
  --after-csv "${DESAN_OUT}/spec-results/CPU2017.*.intrate.test.csv" \
  --after-csv "${DESAN_OUT}/spec-results/CPU2017.*.intspeed.test.csv" \
  --after-csv "${DESAN_OUT}/spec-results/CPU2017.*.fprate.test.csv" \
  --after-csv "${DESAN_OUT}/spec-results/CPU2017.*.fpspeed.test.csv" \
  --core-prefix __msan_warning \
  --core-prefix __msan_maybe_warning \
  --core-prefix __msan_param_ \
  --core-prefix __msan_retval_ \
  --core-prefix __msan_va_arg_ \
  --core-prefix __msan_check_mem_is_initialized \
  --core-prefix __msan_test_shadow \
  --core-prefix __msan_print_shadow \
  --csv-out "${SUMMARY_CSV}" \
  > "${SUMMARY_MD}"

cat "${SUMMARY_MD}"
